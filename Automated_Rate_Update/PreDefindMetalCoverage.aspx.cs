    ...
    public partial class PreDefindMetalCoverage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                try
                {
                    strBatch = SQLStatic.Sessions.GetSessionValue(Request.Cookies["Session_ID"].Value.ToString(), "BATCHID");
                    if (!string.IsNullOrEmpty(strBatch)) // Account Seyp
                        txtEffectiveDates.SelectedDate = Convert.ToDateTime(strBatch);
                }
                catch { }
            }
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Request.Params["catplan"]))
            {
                Response.Redirect("Default4.aspx?id=" + Request.Params["id"], true);
                return;
            }
