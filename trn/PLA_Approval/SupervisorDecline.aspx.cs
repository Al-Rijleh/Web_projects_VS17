...
{
	/// <summary>
	/// Summary description for SupervisorDecline.
	/// </summary>
	public class SupervisorDecline : System.Web.UI.Page
	{
		protected System.Web.UI.WebControls.Label lblCourseTitle;
		protected System.Web.UI.WebControls.Label lbl_fldPla_ApprovalSupervisorDoDecline;
		...
		private bool SaveData(string strMsgList)
		{
		...
		private string reasonMessage()
		{
			...
			for (int i=0;i<chklstReasons.Items.Count;i++)
				if (chklstReasons.Items[i].Selected)
				{
					...
					Reasons = Reasons+","+chklstReasons.Items[i].Value;
				}
		...
		private void lnkbtnManageReasons_Click(object sender, System.EventArgs e)
		{
			...
			lblScript.Text = "<script>popup('/Web_Projects/trn/PLA_Approval/ManageReasons.aspx?type="+strType+"&surl="+strURL+"',500,700)</script>";
		}

        protected void btnSave_Click(object sender, EventArgs e)
        {
            string strMsgList = reasonMessage();
            if (txtMemo.Text == "")
            {
                ShowError("You must fill the reason for declining the request.");
                return;
            }
            if (SaveData(strMsgList))
            {
                if (Request.Params["who"] == "payor")
                ...
                else if (ViewState["Product_Code"].ToString() == "11001")
                {
                    ...
                    lblScript.Text = "<script>alert('Your decision has been saved..');opener.window.location.href='" + cdpUrl + "';window.close()</script>";
                }
                else
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
			...
			else if (ViewState["From_2nd_supervisor"].ToString() == "T")
				Response.Redirect("Supervisor2ndInLineApproval.aspx");
			else
				Response.Redirect("SupervisorApproval.aspx?SkipCheck=YES");
        }

        protected void Button1_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(Request.Params["strMarkCSA"]))
                Response.Redirect("PayorApprovePayment.aspx" + Request.Params["strMarkCSA"]);
            else
                Response.Redirect("PartialPayment.aspx" + Request.Params["strMarkCSA"]);
        }
