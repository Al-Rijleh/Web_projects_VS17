...
{
    public partial class DoubleAgeRate : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Request.Params["catplan"].StartsWith("0"))
            {
                ...
                if (!string.IsNullOrEmpty(session_id))
                {
                    ...
                    user_name = Data.user_name_from_id(Request.Params["id"]);
                            ...
                            foreach (DataRow dr in tbl.Rows)
                            {
                                if (!dr["exist"].ToString().Equals("0"))
                                {
                                    foreach (string str in ar)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(Request.Params["back"]))
                                        {
                                            Data.SaveRate_ver2(Request.Params["accnt"], Request.Params["ver"], dr["class_code"].ToString(), Request.Params["catcode"],
