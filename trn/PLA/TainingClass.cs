	...
	public class TrainingClass
	{
		...
		public static string CourseName(string record_id)
		{
			return SQLStatic.SQL.ExecScaler("select PKG_Training.CourseCodeTitle("+record_id+") from dual",
				Training_Source.TrainingClass.ConnectioString).ToString();
		}	
		...
		public static string GetAccountNumber(string employee_number)
		{
			...
			try
			{
				strAccountNumber=SQLStatic.SQL.ExecScaler(" select pkg_employee.ee_account_number("+employee_number+") from dual",Training_Source.TrainingClass.ConnectioString).ToString();
			}
			catch
			{
				return "";
		...
		public static bool SessionHasTimeOut(string strSessionId)
		{
			...
			return
				SQLStatic.SQL.ExecScaler(strSQL,Training_Source.TrainingClass.ConnectioString).ToString()=="0";
		}

		public static string EmployeeNumberFromRecordID(string record_id)
		{
			return SQLStatic.SQL.ExecScaler("select pkg_training.employee_number_from_record_id("+record_id+") from dual",Training_Source.TrainingClass.ConnectioString).ToString();
		}

		public static string FormId(string header_record_id)
		{
			object ob = SQLStatic.SQL.ExecScaler(
				"select pkg_training.get_form_id("+header_record_id+") from dual",Training_Source.TrainingClass.ConnectioString);
			if (ob!=null)
				return ob.ToString();
			else
				return "";
		...
		{
			object ob = SQLStatic.SQL.ExecScaler(
				"select pkg_training.can_access_other_application("+strUserId+") from dual",Training_Source.TrainingClass.ConnectioString);
			if (ob!=null)
		...
		{
			string strEmployeeNumber = SQLStatic.Sessions.GetSessionValue(strSessionID,"FOUND_EMPLOYEE_NUMBER",Training_Source.TrainingClass.ConnectioString);
		...
		{
            string strEmployeeNumber = SQLStatic.Sessions.GetSessionValue(strSessionID, "APPLICANT_EMPLOYEE_NUMBER");
            if (strEmployeeNumber == "")
                strEmployeeNumber = SQLStatic.Sessions.GetSessionValue(strSessionID, "TRN_EMPLOYEE_NUMBER");
            if (strEmployeeNumber == "")
                return strDefaulrEmployeeNumber;
            else
                return strEmployeeNumber;
            // Maher 4/25/2016
            //string strEmployeeNumber = SQLStatic.Sessions.GetSessionValue(strSessionID,"TRN_EMPLOYEE_NUMBER",Training_Source.TrainingClass.ConnectioString);
            //if (strEmployeeNumber == "")
            //    strEmployeeNumber = SQLStatic.Sessions.GetSessionValue(strSessionID,"APPLICANT_EMPLOYEE_NUMBER",Training_Source.TrainingClass.ConnectioString);
            //if (strEmployeeNumber=="")
            //    return strDefaulrEmployeeNumber;
            //else
            //    return strEmployeeNumber;
		...
		{
			object ob = SQLStatic.SQL.ExecScaler(
				"select pkg_training.has_pending_approval("+strEmployeeNumber+") from dual",Training_Source.TrainingClass.ConnectioString);
			if (ob!=null)
				return ob.ToString()=="1";
			else
				return false;
		...
		{
			return SQLStatic.SQL.ExecScaler("select pkg_training.AquiredPrerequsit("+strEmployeeNumber+","+strProceesingYear+") from dual",Training_Source.TrainingClass.ConnectioString).ToString()=="1";
		}
		...
		{
			return SQLStatic.Sessions.GetSessionValue(session_id,"TRN_READONLY",Training_Source.TrainingClass.ConnectioString).ToString()=="T";			
		}
		
		public static bool IsSuperUser(string session_id,string user_id,string employee_number)
		{
			bool UserIsSuper=SQLStatic.SQL.ExecScaler("select pkg_training_2.issuperuser("+user_id+") from dual",Training_Source.TrainingClass.ConnectioString).ToString() != "0";
			string strSelectFoundEE="";
			if (UserIsSuper)
				strSelectFoundEE = SQLStatic.Sessions.GetSessionValue(session_id,"FOUND_EMPLOYEE_NUMBER",Training_Source.TrainingClass.ConnectioString).ToString();
			if (strSelectFoundEE=="")
				return false;
			else 
			{
		...
		public static string application_Processing_Year(string header_id)
		{
			return SQLStatic.SQL.ExecScaler(
				"select pkg_training_2.application_processing_year("+header_id+") from dual",Training_Source.TrainingClass.ConnectioString).ToString();
		}
		...
		{		
			...
			if (header_record_id!="-1")
				Use2008=Training_Source.TrainingClass.Use2008Types_Needs(header_record_id);
...
//				if (!ItemOk)
					ItemOk =Training_Source.TrainingClass.PageCompleted(strURL,strIncomplatePages);
				if (!ItemOk)
					OneMore=true;
		...
		public static bool Use2008Types_Needs(string header_record_id)
		{
			return SQLStatic.SQL.ExecScaler(
				...
				Training_Source.TrainingClass.ConnectioString).ToString()=="1";
		}
